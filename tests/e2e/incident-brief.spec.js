...
test("release candidate shows the new save checkpoint", async ({ page }) => {
  await page.goto("/incident.html");

  await expect(
    page.getByRole("heading", { name: "Controllo prima del salvataggio" })
  ).toBeVisible();
  await expect(page.getByText("Evidenze server protette")).toBeVisible();
});
